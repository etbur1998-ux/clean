...
class _PrivateCompanyScreenState extends State<PrivateCompanyScreen>
  ...
  int? _weredaId;
  int? _vehicleId;
  List<XFile> _images = []; // receipt photos
  ...
  Future<void> _loadAll() async {
    ...
    try {
      ...
      setState(() {
        ...
        _loading = false;
        // Soft warning — form still works, server auto-creates company
        _loadError = _companyInfo == null
            ? 'Company profile not linked yet. Ask your administrator to link '
                  'your account to a company. Receipt submission will still work if '
                  'your email matches a company profile.'
            : null;
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _loading = false;
        _loadError = 'Connection error. Check your network and try again.';
      });
    }
  }
  ...
  // ── Helper: company display name ──────────────────────────────────────────
  String get _companyDisplayName {
    if (_companyInfo?['company_name'] != null) {
      return _companyInfo!['company_name'].toString();
    }
    // Safe access — don't call Provider inside build getter
    return _currentUserName ?? 'Private Company';
  }

  String? get _currentUserName {
    try {
      return Provider.of<AuthService>(context, listen: false).currentUser?.name;
    } catch (_) {
      return null;
    }
  }
  ...
  Widget _buildSubmitTab(bool isDark) {
    bool companyNotLinked = _companyInfo == null && _loadError != null;
        ...
        child: Column(
          ...
          children: [
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            ...
                            'Ask your administrator to link your account to a company. '
                            'Receipt submission will still work if your email matches a company profile.',
                            style: TextStyle(
            ...
            Container(
              ...
              margin: EdgeInsets.only(bottom: companyNotLinked ? 12 : 20),
              decoration: BoxDecoration(
